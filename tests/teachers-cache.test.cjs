...
const rows=[{short_name:'Тестова А. О.',full_name:'Тестова Анна Олеговна',url:'https://example.org/teacher'}];
...
test('directory fetched once and cached; original names resolve unchanged',async()=>{
 const s=setup();await Promise.all([s.ctx.loadTeachers(),s.ctx.loadTeachers()]);assert.equal(s.calls,1);assert.equal(s.saved.key,'starosta.public-teachers.v1:bgtu');assert.equal(s.saved.value.rows.length,1);assert.equal(vm.runInContext("teacherDirectory[teacherKey('Тестова А.О.')].full",s.ctx),rows[0].full_name);
});
