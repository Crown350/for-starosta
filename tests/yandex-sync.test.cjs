...
function source(schedule=html){
 ...
 return {calls,send:async(url,options)=>{
  ...
  if(form.get('namedata')==='group')return {headers:{},text:'<option value="ТЕСТ-01">ТЕСТ-01</option>'};
  assert.equal(form.get('group'),'ТЕСТ-01');assert.equal(form.get('form'),'очная');
  return {headers:{},text:schedule};
 }};
}
test('Yandex parser preserves session and returns 25 real fixture lessons',async()=>{
 const s=source();const data=await fetchSchedule(s.send,'ТЕСТ-01');assert.equal(data.lessons.length,25);assert.equal(s.calls.length,3);assert.equal(data.lessons.find(r=>r.dow===6).pair,8);
});
test('invalid markup or fewer than ten lessons never publishes a snapshot',async()=>{
 for(const html of ['unavailable','<table class="contless"><tr><td class="daeweek">Понедельник</td></tr><tr><td class="schtime">08:00 - 09:35</td><td class="schname">Алгебра</td></tr></table>']){
  const s=source(html);await assert.rejects(sync({env:{GROUP:'ТЕСТ-01',SUPABASE_URL:'https://test.supabase.co',SUPABASE_SERVICE_ROLE_KEY:'test-key'},send:async(url,options)=>{if(url.includes('claim_yandex_schedule'))return {text:JSON.stringify({acquired:true,cache:{request_token:'test'}})};if(url.includes('finish_yandex_schedule')){const b=JSON.parse(options.body);assert.ok(b.failure);assert.equal(b.snapshot,undefined);return {text:JSON.stringify({cache:{}})};}return s.send(url,options);}}).then(r=>{throw Error(r.error)}),/разметка|минимум 10/);
...
test('hash ignores object key order; unchanged content still records successful check',async()=>{
 ...
 const s=source();const previous=await fetchSchedule(s.send,'ТЕСТ-01');let published;
 ...
 const result=await sync({env:{GROUP:'ТЕСТ-01',SUPABASE_URL:'https://test.supabase.co',SUPABASE_SERVICE_ROLE_KEY:'sb_secret_test'},send,now:()=> '2026-09-10T10:00:00Z'});
...
test('cached claim does not issue any BGTU request',async()=>{
 const calls=[];
 const result=await sync({env:{GROUP:'ТЕСТ-01',SUPABASE_URL:'https://test.supabase.co',SUPABASE_SERVICE_ROLE_KEY:'test'},send:async(url)=>{
...
test('missing group fails before contacting BGTU',async()=>{let calls=0;await assert.rejects(fetchSchedule(async()=>{calls++;},''),/GROUP/);assert.equal(calls,0);});
