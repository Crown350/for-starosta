...
(async()=>{
  ...
  const group=process.env.GROUP||current.group;
  if(!data.ok||data.mock||!group||data.group?.toLowerCase()!==group.toLowerCase()||!data.lessons?.length||!Number.isFinite(Date.parse(data.fetchedAt)))throw new Error('Invalid snapshot');
