...
(()=>{
  ...
  window.cloudScheduleBlocked=()=>!!token && (conflict||!usePublicSchedule);
  ...
  document.getElementById('cloud-login').onsubmit=async e=>{
    e.preventDefault();sessionEpoch++;token=document.getElementById('cloud-key').value.trim();document.getElementById('cloud-key').value='';status('Проверяю ключ…');
    panel.setAttribute('aria-busy','true');
    try{const session=await api('/api/session');usePublicSchedule=session.usePublicSchedule!==false;status('Загружаю журнал…');await pull();}catch(error){token='';ready=false;status(error.message);permissions();}finally{panel.removeAttribute('aria-busy');}
  };
  ...
  document.getElementById('cloud-logout').onclick=async()=>{if(writing)return status('Дождись завершения сохранения');if(dirty&&!await askConfirm('Выйти с несохранёнными изменениями? Сначала можно скачать их.'))return;sessionEpoch++;refreshPending=null;token='';ready=false;dirty=false;conflict=false;usePublicSchedule=true;clearTimeout(timer);S=fresh();localBackup=null;view=null;ctx={};offline.checked=false;permissions();render();try{await localCopy.logout();status('Выход выполнен · журнал и копия устройства очищены');}catch{privacyError();}};
