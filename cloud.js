...
(()=>{
  ...
  document.getElementById('cloud-login').onsubmit=async e=>{
    ...
    try{const session=await api('/api/session');usePublicSchedule=session.usePublicSchedule!==false;await pull();}catch(error){token='';ready=false;status(error.message);permissions();}
  };
